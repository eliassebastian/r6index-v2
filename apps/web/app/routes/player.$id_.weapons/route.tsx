...
import { WeaponFilter } from '~/data/filters/filter-weapons';
...
export const meta: MetaFunction = () => {
	return [{ title: 'R6 Index: Player Weapons' }];
};
...
export default function PlayerWeaponsRoute() {
			...
			<Await resolve={data}>
				{(data) => (
					<PlayerDetailedPage.Container>
						<PlayerDetailedPage.Filter data={data?.weapons} config={WeaponFilter} title='Weapons' useSide>
							{(data) => (
								<PlayerDetailedPage.Table label='weapon table' data={data} defaultSortColumn='weaponName'>
									{(data) => (
										<>
											<TableHeader className='!bg-accent-2'>
												<Column id='icon' width={100} />
												<Column className={'font-normal text-xs'} id='weaponName' isRowHeader allowsSorting width={175}>
													Name
												</Column>
												<Column className={'font-normal text-xs'} id='weaponType' width={175}>
													Type
												</Column>
												<Column
											...
											>
												{(row) => {
													return (
														...
														>
															<Cell>
																<div className='w-full text-center text-accent-11'>
																	{UbisoftWeaponsConfig[row.weaponName]?.icon_url ? (
																		<img
																			src={`https://staticctf.akamaized.net/J3yJr34U2pZ2Ieem48Dwy9uqj5PNUQTn/${
																				UbisoftWeaponsConfig[row.weaponName]?.icon_url
																			}`}
																			alt={`${UbisoftWeaponsConfig[row.weaponName]} icon`}
																		/>
																	) : (
																		<span className=''>-</span>
																	)}
															...
															<Cell>{row.weaponName}</Cell>
															<Cell>
																<div className='rounded-lg bg-accent-6 w-fit px-2 py-0.5'>{row.weaponType}</div>
															</Cell>
