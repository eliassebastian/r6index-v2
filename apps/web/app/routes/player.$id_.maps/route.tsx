...
import { MapsFilter } from '~/data/filters/filter-maps';
...
export const meta: MetaFunction = () => {
	return [{ title: 'R6 Index: Player Maps' }];
};
...
export default function PlayerMapsRoute() {
			...
			<Await resolve={data}>
				{(data) => (
					<PlayerDetailedPage.Container>
						<PlayerDetailedPage.Filter data={data?.maps} config={MapsFilter} title='Maps' useSide>
							{(data) => (
								<PlayerDetailedPage.Table label='maps table' data={data} defaultSortColumn='name'>
									{(data) => (
										<>
											<TableHeader className='!bg-accent-2'>
												<Column className={'font-normal text-xs'} id='name' isRowHeader allowsSorting width={175}>
													Name
												</Column>
												<Column
